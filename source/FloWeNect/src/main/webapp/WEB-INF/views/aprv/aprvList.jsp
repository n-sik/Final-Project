  ...
  <div class="aprv-container">
    ...
    <header class="aprv-top" aria-label="전자결재 상단">
      <div class="aprv-top-left">
        ...
        <p class="aprv-top-sub">
          <c:choose>
            <c:when test="${empty param.box || param.box=='mine'}">내 문서함: 내가 작성한 결재 문서를 조회 합니다.</c:when>
            <c:when test="${param.box=='pending'}">결재 대기함: 내가 결재해야 할 문서를 조회 합니다.</c:when>
            <c:when test="${param.box=='processed'}">결재 처리함: 내가 처리한 결재 문서 이력을 조회 합니다.</c:when>
            <c:when test="${param.box=='ref'}">수신/참조함: 수신/참조 문서를 조회 합니다.</c:when>
            <c:otherwise>내 문서함: 내가 작성한 결재 문서를 조회 합니다.</c:otherwise>
          </c:choose>
        </p>
      </div>
      ...
      <div class="aprv-top-right">
        <nav class="aprv-top-tabs" aria-label="전자결재 문서함 탭">
          <a class="aprv-tab ${empty param.box || param.box=='mine' ? 'is-active' : ''}"
             href="${pageContext.request.contextPath}/aprv/readList?box=mine">내 문서함</a>
          <sec:authorize access="hasRole('LEADER')">
            <a class="aprv-tab ${param.box=='pending' ? 'is-active' : ''}"
               href="${pageContext.request.contextPath}/aprv/pendingList?box=pending">대기함</a>
            <a class="aprv-tab ${param.box=='processed' ? 'is-active' : ''}"
               href="${pageContext.request.contextPath}/aprv/processedList?box=processed">처리함</a>
          </sec:authorize>
          <a class="aprv-tab ${param.box=='ref' ? 'is-active' : ''}"
             href="${pageContext.request.contextPath}/aprv/readList?box=ref">참조함</a>

          <sec:authorize access="hasRole('LEADER')">
            <a class="aprv-tab"
               href="${pageContext.request.contextPath}/aprv/asset/manage">서명/직인</a>
          </sec:authorize>
        </nav>
    ...
    <div class="aprv-list-grid" aria-label="전자결재 본문">
      ...
      <section class="aprv-card aprv-card--rail" aria-label="검색 조건">
        ...
        <div class="aprv-card__body aprv-card__body--scroll">
          <form class="aprv-filter"
                action="${pageContext.request.contextPath}/aprv/readList"
                method="get">
            <input type="hidden" name="box" value="${empty param.box ? 'mine' : param.box}"/>
            ...
            <div class="aprv-filter__actions aprv-filter__actions--rail">
              ...
              <a class="aprv-btn"
                 href="${pageContext.request.contextPath}/aprv/readList?box=${empty param.box ? 'mine' : param.box}">초기화</a>
            </div>
          </form>
        </div>
      </section>
                  ...
                  <table class="aprv-table">
                    ...
                    <tbody>
                      <c:forEach var="d" items="${docs}" varStatus="st">
                        <!-- JS 유지: .aprv-item / .aprv-item--clickable / data-href 그대로 -->
          ...
          <div class="aprv-card__footer">
            <div class="aprv-pagination" aria-label="페이지네이션">
              <c:if test="${page.hasPrev}">
                <a class="aprv-page"
                   href="${pageContext.request.contextPath}/aprv/readList?box=${empty param.box ? 'mine' : param.box}&page=${page.prevPage}&fromDt=${fromDt}&toDt=${toDt}&docStatCd=${docStatCd}&formCd=${formCd}&aprvNo=${aprvNo}&aprvTtlPrefix=${aprvTtlPrefix}">이전</a>
              </c:if>

              <c:forEach var="p" begin="${page.startPage}" end="${page.endPage}">
                <a class="aprv-page <c:if test='${p==page.page}'>is-active</c:if>"
                   href="${pageContext.request.contextPath}/aprv/readList?box=${empty param.box ? 'mine' : param.box}&page=${p}&fromDt=${fromDt}&toDt=${toDt}&docStatCd=${docStatCd}&formCd=${formCd}&aprvNo=${aprvNo}&aprvTtlPrefix=${aprvTtlPrefix}">
                  ${p}
                </a>
              </c:forEach>
              ...
              <c:if test="${page.hasNext}">
                <a class="aprv-page"
                   href="${pageContext.request.contextPath}/aprv/readList?box=${empty param.box ? 'mine' : param.box}&page=${page.nextPage}&fromDt=${fromDt}&toDt=${toDt}&docStatCd=${docStatCd}&formCd=${formCd}&aprvNo=${aprvNo}&aprvTtlPrefix=${aprvTtlPrefix}">다음</a>
              </c:if>
            </div>
          </div>
        </c:if>
