  ...
  <div class="aprv-container">
    ...
    <header class="aprv-top" aria-label="전자결재 상단">
      <div class="aprv-top-left">
        ...
        <p class="aprv-top-sub">결재 처리함: 내가 처리한 결재 문서 이력을 조회 합니다.</p>
      </div>
      ...
      <div class="aprv-top-right">
        <nav class="aprv-top-tabs" aria-label="전자결재 문서함 탭">
          ...
          <sec:authorize access="hasRole('LEADER')">
            <a class="aprv-tab" href="${pageContext.request.contextPath}/aprv/pendingList?box=pending">대기함</a>
            <a class="aprv-tab is-active" href="${pageContext.request.contextPath}/aprv/processedList?box=processed">처리함</a>
          </sec:authorize>
    ...
    <div class="aprv-list-grid" aria-label="전자결재 본문">
      ...
      <section class="aprv-card aprv-card--rail" aria-label="검색 조건">
        ...
        <form class="aprv-filter" action="${pageContext.request.contextPath}/aprv/processedList" method="get">
          <input type="hidden" name="box" value="processed"/>
          ...
          <div class="aprv-filter__actions aprv-filter__actions--rail">
            ...
            <a class="aprv-btn" href="${pageContext.request.contextPath}/aprv/processedList?box=processed">초기화</a>
          </div>
        </form>
        </div>
      </section>
          ...
          <div class="aprv-card__footer">
            <div class="aprv-pagination" aria-label="페이지네이션">
              <c:if test="${page.hasPrev}">
                <a class="aprv-page"
                   href="${pageContext.request.contextPath}/aprv/processedList?box=processed&page=${page.prevPage}&fromDt=${fromDt}&toDt=${toDt}&docStatCd=${docStatCd}&formCd=${formCd}&aprvNo=${aprvNo}&aprvTtlPrefix=${aprvTtlPrefix}">이전</a>
              </c:if>

              <c:forEach var="p" begin="${page.startPage}" end="${page.endPage}">
                <a class="aprv-page <c:if test='${p==page.page}'>is-active</c:if>"
                   href="${pageContext.request.contextPath}/aprv/processedList?box=processed&page=${p}&fromDt=${fromDt}&toDt=${toDt}&docStatCd=${docStatCd}&formCd=${formCd}&aprvNo=${aprvNo}&aprvTtlPrefix=${aprvTtlPrefix}">
                  ${p}
                </a>
              </c:forEach>
              ...
              <c:if test="${page.hasNext}">
                <a class="aprv-page"
                   href="${pageContext.request.contextPath}/aprv/processedList?box=processed&page=${page.nextPage}&fromDt=${fromDt}&toDt=${toDt}&docStatCd=${docStatCd}&formCd=${formCd}&aprvNo=${aprvNo}&aprvTtlPrefix=${aprvTtlPrefix}">다음</a>
              </c:if>
            </div>
          </div>
        </c:if>
